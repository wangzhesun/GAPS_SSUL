...
def copy_and_paste(novel_img_chw, novel_mask_hw, base_img_chw, base_mask_hw, mask_id):
    ...
    novel_mask_hw = tr_F.resize(novel_mask_hw, (target_H, target_W),
    ...
    base_img_chw[:, paste_y:paste_y + h, paste_x:paste_x + w][:, novel_mask_hw] = novel_img_chw[:,
                                                                                  novel_mask_hw]
...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='voc', choices=['voc', 'ade', 'coco'],
...
def get_dataset(opts, seed):
    ...
    dataset_dict['vanilla_train'] = dataset(opts=opts, image_set='train',
                                            transform=vanilla_transform, cil_step=opts.curr_step)
    ...
    if opts.curr_step > 0 and opts.mem_size > 0:
        ...
        dataset_dict['vanilla_memory'] = dataset(opts=opts, image_set='memory',
...
class gaps_synthesizer:
    ...
    def apply_gaps(self, augmented_base_img, augmented_base_label):
        ...
        for b in range(augmented_base_label.shape[0]):
            ...
            new_img_chw, new_mask_hw = self.apply_gaps_single(cur_img_chw, cur_mask_hw,
                                                              novel_obj_id)
...
def main(opts, seed):
    ...
    if opts.curr_step >= 1:
        assert os.path.isfile(opts.ckpt), f"{opts.ckpt} not found!"
    ...
    dataset_dict = get_dataset(opts, seed)
